...
def Ithr(phi_finer,molt):
    
    # ~ I_suigreatb0_finer_5= (r - phi_finer*P_c*(1+molt)/2.)/kappa * (1 + S_0/np.unique(K_D))
    I_suigreatb0_finer_5= (r - phi_finer*P_c*(1+molt*molt)/(1+molt))/kappa * (1 + S_0/np.unique(K_D))

    # ~ I_ruigreatb0_finer_5= (r - np.minimum(molt,1.)*phi_finer*P_c/2)/kappa * (1 )
    I_ruigreatb0_finer_5= (r - molt*phi_finer*P_c/(1+molt))/kappa * (1 )
    Ithr = np.maximum( I_ruigreatb0_finer_5, I_suigreatb0_finer_5)
...
def plot_summary_diagram(phi, I, array_z , zlabel, fig, ax):
    ...
    ax.plot(phi_finer,I_1,color='g',linestyle='--', linewidth=3)
    ax.plot(phi_finer,I_5,color='teal',linestyle='--', linewidth=3)
    ax.plot(phi_finer,I_1o5,color='lightgreen',linestyle='--', linewidth=3)
...
grid = fig.add_gridspec(nrows=1,ncols=2, left=0.07, right=0.95, top=0.96, bottom=0.15,
...
for ip, pstr in enumerate(pstr_list):
    ...
    I_5   = Ithr(phi_finer,5)
    I_1o5 = Ithr(phi_finer,1./5)
    I_1 = Ithr(phi_finer,1.)
...
out_file='{out}/FigS3.pdf'.format(out=dir_out_plots_tot)
